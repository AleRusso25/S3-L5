...
let resultDice = dice();
console.log("il numero random tra 1 e 6 è", resultDice);
...
// non sono riuscita a rendere maiuscole la iniziali delle parole

/*   return word.charAt(0).toUpperCase() + word.charAt(1).toLowercare();
  .map(function (word) {
})*/

console.log(splitMe("Ciao sono Ale"));
...
console.log(onlyLetters("j123a123v567a876"));

/* ESERCIZIO 6
  Crea una funzione chiamata "isThisAnEmail" che riceve una stringa come parametro e ritorna true se la stringa è un valido indirizzo email.
*/

// HO PROVATO A FARE COSI MA NON SAREBBE EFFETTIVAMENTE UN INDIRIZZO VALIDO , SU INTERNET HO TROVATO IL METODO COMPLETO
...
console.log(isThisAnEmail("ale.russo@gmail.com"));

/* ESERCIZIO 7
  Scrivi una funzione chiamata "whatDayIsIt" che ritorna il giorno della settimana corrente.
*/
function whatDayIsIt() {
  let days = ["Domecica", "lunedi", "Martedì", "Mercoledì", "Giovedì", "Venerdì", "Sabato"];
  let today = new Date();
...
function rollTheDices(num) {
  ...
  for (let i = 0; i < num; i++) {
    let value = dice();
    resultDice.valore.push(value);
    resultDice.somma += value;
  }
  return resultDice;
}
let rollsDice = rollTheDices(4);
console.log(rollsDice.valore);
console.log("La somma è:" + rollsDice.somma);
...
function howManyDays(pastDate) {
  ...
  if (isNaN(past)) {
    ...
    const differenceInMs = currentDate - past;
    const differenceInDays = Math.floor(differenceInMs / (1000 * 60 * 60 * 24));
    return differenceInDays;
  }
}
console.log(howManyDays("2024/08/20"));
...
const ObjectNoCity = deleteProp(myObject, "city");
console.log(ObjectNoCity);

/* ESERCIZIO 12
  Scrivi una funzione chiamata "newestMovie" che trova il film più recente nell'array "movies" fornito.
*/
function newestMovie(movies) {
  return movies.reduce((mostRecent, currentMovie) => {
    return parseInt(currentMovie.Year) > parseInt(mostRecent.Year) ? currentMovie : mostRecent;
  });
}
...
function onlyLastMillennium(movies) {
  return movies.filter((movies) => {
    return parseInt(movies.Year) >= 1001 && parseInt(movies.Year) <= 2000;
  });
}
